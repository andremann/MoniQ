...
public class ControlDAO implements GenericControlDAO {
	...
	@Override
	public List<Control> listActiveControls(final String scenario) {
		log.debug("List active controls in scenario " + scenario);
		String query = "SELECT * FROM controls WHERE status = 'active' AND monitoringscenario = ?";
		return queryForControlList(query, scenario);
	}

	@Override
	public List<Control> listActiveControls(final String scenario, final String metric) {
		log.debug("List active controls in scenario " + scenario + " for metric " + metric);
		String query =
				"SELECT * FROM controls c1 WHERE status = 'active' AND monitoringscenario = ? AND ? in (SELECT json_array_elements(selectors)->>'metric' FROM controls c2 WHERE c1.id = c2.id);";
		return queryForControlList(query, scenario, metric);
	}
