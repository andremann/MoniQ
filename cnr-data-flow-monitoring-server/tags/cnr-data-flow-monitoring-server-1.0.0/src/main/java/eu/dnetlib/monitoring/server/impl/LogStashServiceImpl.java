		...
		server.addListener(new Listener() {
			...
			public void received(final Connection connection, final Object object) {
				if (object instanceof Observation) {
					boolean ret = storeObservation((Observation) object);
					if (!ret) {
						log.info(String.format("Cannot store observation -> %s(%s = %s)", ((Observation) object).getSensorType(), ((Observation) object).getMetric(),
								((Observation) object).getValue()));
					}
				} else {
