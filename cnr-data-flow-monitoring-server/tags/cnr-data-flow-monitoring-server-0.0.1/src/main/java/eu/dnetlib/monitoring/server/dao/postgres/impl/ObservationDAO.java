...
public class ObservationDAO implements GenericStashDAO {
	...
	@Override
	public boolean create(final Observation record) {
		if (record.getLog() == null) return false;
		log.info("Serving stash record from " + record.getSensorType());
		try {
			jdbcTemplate.update(
					...
					record.getMetric(),
					MapToPGJson(record.getLabels()),
					record.getSensorType(),
					record.getSensorId(),
					record.getLog(),
					record.getMonitoringScenario());
	...
	private Map PGJsonToMap(final Object obj) {
		if (obj != null) return new Gson().fromJson(((PGobject) obj).getValue(), Map.class);
