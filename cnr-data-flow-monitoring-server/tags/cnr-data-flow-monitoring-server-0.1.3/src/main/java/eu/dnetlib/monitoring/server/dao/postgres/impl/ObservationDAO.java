...
public class ObservationDAO implements GenericStashDAO {
	...
	public boolean create(final Observation observation) {
		...
		if (observation.getLog() == null) { return false; }
		try {
			jdbcTemplate.update(
	...
	public List<Observation> findKLastObservations(final String scenario, final String metric, final int k) {
		String query =
				"SELECT * FROM (SELECT * from logstash where monitoringscenario = ? AND metric = ? ORDER BY timemarker DESC LIMIT ?) as t ORDER BY timemarker ASC";
	...
	public List<Observation> findKLastObservations(final String scenario, final String metric, final String labelName, final int k) {
		String query =
				"SELECT * FROM (SELECT * FROM logstash WHERE monitoringscenario = ? AND metric = ? AND labels ->> ? != '' ORDER BY timemarker DESC LIMIT ?) as t ORDER BY timemarker ASC";
